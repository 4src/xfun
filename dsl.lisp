...
;; lambda macros
(set-macro-character 
   #\\ (lambda (s _) `(lambda (,(read s)) ,(read s))))

(defstruct it) ; base struct

(defmethod print-object ((self it) stream)
  (format stream "#<~A" (type-of self))
  (dolist (slot (slots self))
    (unless (char= (char (string slot) 0) #\_)
      (format stream " ~A=~S" slot
              (slot-value self slot))))
  (format stream ">"))

(defmacro defthing (name &rest slots)
  (let* ((slots (mapcar \s  (if (listp s) (car s) s)) slots)
         (make (intern (format nil "%MAKE-~{~A~}" name))))
    `(progn
       (defstruct (,name (:include it) (:constructor ,make)) ,@slots)
       (defmethod slots ((self ,name)) ',slots))))

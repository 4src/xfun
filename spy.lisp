...
(defmethod smo ((self data) &optional (score (lambda (b r) (- b r))))
  (labels
    ((_like (row best rest &aux (n (+ (length (data-rows best)) 
                                      (length (data-rows rest)))))
             (funcall score (like best row :nall n :nh 2) 
                            (like rest row :nall n :nh 2)))
     (_score (todo best rest &aux (stop (floor (* (length todo) !top))))
             (subseq (sort todo #'> :key (lambda (r) (_like r best rest ))) 0 stop))
     (_sort (rows) (clone-data self rows t))
     (_smo (budget todo done &aux (sorted-rows (data-rows (_sort done))))
           (if (or (< budget 1) (<= (length todo) 3))
             (first sorted-rows)
             (let* ((cut (floor (expt (length sorted-rows) !best)))
                    (todo (_score todo (_sort (subseq sorted-rows 0 cut)) 
                                       (_sort (subseq sorted-rows cut)))))
               (push (pop todo) done)
               (_smo (1- budget) todo done)))))
    (nshuffle $rows)
    (_smo (- !stop !start) (subseq $rows !start) 
                           (subseq $rows 0 !start))))

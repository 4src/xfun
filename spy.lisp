...
(defmethod smo ((self data) &optional (score (lambda (b r) (- b r))))
  (labels
    ((like_ (row best rest &aux (n (+ (length (data-rows best)) 
                                      (length (data-rows rest)))))
             (funcall score (like best row :nall n :nh 2) 
                            (like rest row :nall n :nh 2)))
     (score_ (todo best rest &aux (stop (floor (* (length todo) !top))))
             (subseq (sort todo #'> :key (lambda (r) (like_ r best rest ))) 0 stop))
     (sort_ (rows) (clone-data self rows t))
     (smo_ (budget todo done &aux (sorted-rows (data-rows (sort_ done))))
           (if (or (< budget 1) (<= (length todo) 3))
             (first sorted-rows)
             (let* ((cut (floor (expt (length sorted-rows) !best)))
                    (todo (score_ todo (sort_ (subseq sorted-rows 0 cut)) 
                                       (sort_ (subseq sorted-rows cut)))))
               (push (pop todo) done)
               (smo_ (1- budget) todo done)))))
    (nshuffle $rows)
    (smo_ (- !stop !start) (subseq $rows !start) 
                           (subseq $rows 0 !start))))
